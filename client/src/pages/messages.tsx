...
export default function Messages() {
  ...
  const { data: suggestedUsers = [] } = useQuery({
    ...
    select: (data: any[]) => data.filter((artist: any) => artist.id !== user?.id).slice(0, 5)
  }) as { data: any[] };
  ...
  const createConversationMutation = useMutation({
    mutationFn: async (participant2Id: string) => {
      const response = await apiRequest("POST", "/api/conversations", { participant2Id });
      return response.json();
    },
                  ...
                  <DialogContent className="sm:max-w-md glass-effect border-white/10">
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        ...
                        {/* Search Results */}
                        {newConversationUser.length > 1 && searchResults.length > 0 && (
                            ...
                            {searchResults.map((searchUser: any) => (
                              ...
                              >
                                <Avatar className="h-10 w-10">
                                  <AvatarImage src={searchUser.profileImage} alt={searchUser.username} />
                      ...
                      <div className="space-y-3">
                        <h4 className="text-sm font-medium text-muted-foreground">Suggested Users</h4>
                        {suggestedUsers.map((artist: any) => (
                          <div key={artist.id} className="flex items-center justify-between p-3 rounded-lg glass-effect border-white/10">
                            <div className="flex items-center space-x-3">
                              <Avatar className="h-10 w-10">
                                <AvatarImage src={artist.profileImage} alt={artist.username} />
                                <AvatarFallback className="bg-gradient-to-r from-pink-500 to-orange-500">
                ...
                {conversations.length === 0 ? (
                  <div className="p-6 text-center">
                    ...
                    <p className="text-muted-foreground mb-4">
                      Start a conversation with other artists to collaborate, share music, or just chat!
                    </p>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {getOtherParticipant(selectedConversation)?.role || "Artist"}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex-1 flex items-center justify-center p-8">
                  <div className="text-center max-w-md">
                    ...
                    <p className="text-muted-foreground mb-6">
                      Connect with artists, collaborate on projects, and build your music community. Select a conversation from the sidebar to get started.
                    </p>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      <div className="text-center">
                        ...
                        <p className="text-xs text-muted-foreground">Work together on tracks</p>
                      </div>
                      <div className="text-center">
                        ...
                        <p className="text-xs text-muted-foreground">Exchange tracks and ideas</p>
                      </div>
                    </div>
