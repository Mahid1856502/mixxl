...
export async function validateAudioUrl(url: string): Promise<boolean> {
  try {
    ...
    if (!response.ok) {
      console.warn('Audio URL validation failed - response not ok:', response.status);
      return false;
    }
    ...
    console.log('Audio URL validation result:', { url, isValidAudio, contentType });
    return isValidAudio;
  } catch (error) {
...
export function getAudioErrorMessage(errorType: PlaybackErrorType): { title: string; description: string } {
  switch (errorType) {
